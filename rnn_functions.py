...
def IndRNNTanhCell(input, hidden, w_ih, w_hh, b_ih=None, b_hh=None):
    hy = F.tanh(F.linear(input, w_ih, b_ih) + F.mul(w_hh, hidden))
    return hy


def IndRNNReLuCell(input, hidden, w_ih, w_hh, b_ih=None, b_hh=None):
...
def AutogradRNN(mode, input_size, hidden_size, num_layers=1, batch_first=False,
    ...
    if mode == 'RNN_RELU':
        ...
        cell = GRUCell
    elif mode == 'INDRNN_RELU':
        cell = IndRNNReLuCell
    elif mode == 'INDRNN_TANH':
        cell = IndRNNTanhCell
    else:
