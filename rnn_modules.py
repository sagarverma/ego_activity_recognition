...
from rnn_functions import RNN as backendRNN
    ...
    def __init__(self, mode, input_size, hidden_size,
        ...
        for layer in range(num_layers):
            for direction in range(num_directions):
                ...
                if 'IND' in mode:
                    w_hh = Parameter(torch.Tensor(gate_size))
                else:
        ...
        for name, weight in self.named_parameters():
            if 'weight_hh' in name:
                if self.hidden_min_abs:
                    abs_kernel = torch.abs(weight.data)
                    min_abs_kernel = torch.clamp(abs_kernel, min=self.hidden_min_abs)
                    weight.data.copy_(
                        torch.mul(torch.sign(weight.data), min_abs_kernel))
                if self.hidden_max_abs:
                    weight.data.copy_(
                        torch.clamp(weight.data, max=self.hidden_max_abs,
    ...
    def __init__(self, *args, **kwargs):
        if 'nonlinearity' in kwargs:
            if kwargs['nonlinearity'] == 'tanh':
                ...
                mode = 'RNN_RELU'
            elif kwargs['nonlinearity'] == 'indtanh':
                mode = 'INDRNN_TANH'
            elif kwargs['nonlinearity'] == 'indrelu':
                mode = 'INDRNN_RELU'
            else:
